// Same shape as Room Booking's IdentityScreen.jsx (CGL-email-only
// identity, no password) — kept as its own component rather than a
// shared one because the two check different approver lists and carry
// different branding; the underlying logic (email validation, name
// parsing) is small enough that duplicating it here is simpler than
// generalising IdentityScreen.jsx to take a "which approvers" prop.
import { useState } from "react";
import { Link } from "react-router-dom";
import { CGL } from "../../data/rooms.js";
import { CAR_APPROVERS } from "../../data/car.js";
...
function CarIdentityScreen({ onIdentify }) {
  ...
  async function proceed() {
    ...
    let isApprover = CAR_APPROVERS.some(a => norm(a.email) === norm(trimmed));
    // Same additive staff-directory check as Room Booking's
    // IdentityScreen.jsx — admins added via the Staff Portal directory
    // get approver rights here too. Never blocks continuing if this fails.
    if (!isApprover) {
      setChecking(true);
      try {
      ...
      } catch (e) { console.error("Staff directory approver check failed (falling back to CAR_APPROVERS only)", e); }
      setChecking(false);
    }
      ...
      <div style={{ background: "rgba(255,255,255,0.97)", borderRadius: 20, maxWidth: 420, width: "100%", overflow: "hidden", boxShadow: "0 32px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ background: CGL.blackcurrant, padding: "32px 32px 28px", textAlign: "center", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ color: CGL.orchid, fontSize: 13, fontWeight: 600, letterSpacing: 1.5 }}>CAR BOOKING</div>
          </div>
        </div>
...
export default CarIdentityScreen;
