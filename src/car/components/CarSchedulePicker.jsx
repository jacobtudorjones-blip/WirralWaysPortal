// Drag-to-select time picker for booking the car — same interaction as
// Room Booking's DaySchedulePicker.jsx, trimmed down for one vehicle (no
// room concept, no waitlist button — with only one car, "waitlist for
// this slot" doesn't really apply the same way).
...
// dayBookings: this date's pending/confirmed bookings only (already
// filtered by the caller) — cancelled/rejected ones don't block a slot.
function CarSchedulePicker({ date, dayBookings, startTime, endTime, onChange }) {
  ...
  function isConfirmed(slot) {
    ...
    return confirmedBookings.some(b => slotToMins(b.start_time) < em && slotToMins(b.end_time) > sm);
  }
  function isPending(slot) {
    const sm = slotToMins(slot), em = sm + 30;
    return !isConfirmed(slot) && pendingBookings.some(b => slotToMins(b.start_time) < em && slotToMins(b.end_time) > sm);
  }
  ...
  function slotLabel(slot) {
    const sm = slotToMins(slot), em = sm + 30;
    const bk = [...confirmedBookings, ...pendingBookings].find(b => slotToMins(b.start_time) < em && slotToMins(b.end_time) > sm);
    return bk ? bk.requested_by.split(" ")[0] : null;
  }
...
export default CarSchedulePicker;
